...
impl FTransform {
    pub fn to_matrix_with_scale(self) -> FMatrix {
        ...
        matrix.m[3][0] = self.translation.x as f32;
        matrix.m[3][1] = self.translation.y as f32;
        matrix.m[3][2] = self.translation.z as f32;
        ...
        matrix.m[0][0] = (1.0 - (yy2 + zz2)) * self.scale3d.x as f32;
        matrix.m[1][1] = (1.0 - (xx2 + zz2)) * self.scale3d.y as f32;
        matrix.m[2][2] = (1.0 - (xx2 + yy2)) * self.scale3d.z as f32;

        let yz2 = self.rotation.y * z2;
        let wx2 = self.rotation.w * x2;
        matrix.m[2][1] = (yz2 - wx2) * self.scale3d.z as f32;
        matrix.m[1][2] = (yz2 + wx2) * self.scale3d.y as f32;

        let xy2 = self.rotation.x * y2;
        let wz2 = self.rotation.w * z2;
        matrix.m[1][0] = (xy2 - wz2) * self.scale3d.x as f32;
        matrix.m[0][1] = (xy2 + wz2) * self.scale3d.y as f32;

        let xz2 = self.rotation.x * z2;
        let wy2 = self.rotation.w * y2;
        matrix.m[2][0] = (xz2 + wy2) * self.scale3d.x as f32;
        matrix.m[0][2] = (xz2 - wy2) * self.scale3d.z as f32;
